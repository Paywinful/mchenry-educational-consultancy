    ...
    <div>
      ...
      <div className="w-full px-8 py-8 bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
          {[
            ['Sign Up & Consultation', 'Simply input your unique identification number to begin the process'],
            ['Select Programs & Apply', 'We advise on schools, deadlines, and complete your application'],
            ['Visa & Travel Assistance', 'Document guidance, embassy liaison, flight booking'],
            ['Accommodation & Pre-Departure', 'Arrange housing, cultural orientation'],
          ].map(([title, desc], index) => (
            <div key={index} className="bg-[#D9D9D9] p-10 rounded-xl shadow-md text-center">
      ...
      <div className="w-full bg-[#D9D9D9] px-8 py-10 flex-col justify-center">
        ...
        <div className="bg-white my-8 lg:mx-64 rounded-md shadow-xl p-4 flex space-x-4 border-[#ACACAC]">
          <input
            placeholder="Program or Course"
            className="border border-[#ACACAC] p-2 rounded-sm w-84 text-sm"
          />
          <button className="border border-[#ACACAC] p-2 text-sm rounded-sm font-bold hover:cursor-pointer hover:bg-[#D9D9D9]">
            Select Location
          </button>
          <button className="bg-[#f0b22b] text-white p-2 w-48 text-sm rounded-sm hover:cursor-pointer hover:bg-[#a9791c] transition">
            Search Programs
          </button>
        </div>
      </div>
      ...
      {/* Numbers Section */}
      <div className="w-full bg-[#202020] px-12 py-12 flex justify-between md:px-36">
        {[
          ['50+', 'Partner Schools'],
          ['95%', 'Success rate'],
          ...
          <div key={index} className="flex flex-col items-center">
            <h1 className="text-[#f0b22b] text-4xl font-black">{value}</h1>
