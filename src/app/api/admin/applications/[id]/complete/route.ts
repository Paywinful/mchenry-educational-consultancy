...
export async function POST(
  ...
  // Gate: use your helper, but don't export it from this file
  const gate = await getAdminClient() as any;
  if (gate?.error) return gate.error as Response;
  const { supabase } = gate as { supabase: any };
  ...
  // 2) Find earliest pending/overdue payment
